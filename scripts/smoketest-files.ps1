##############################################################################
# This is a Windows PowerShell script for smoketesting the 'files' canister  #
#                                                                            #
# What is special is that we verify that files are persisted across upgrades #
##############################################################################
Write-Host " "
Write-Host "--------------------------------------------------"
...
Write-Host " "
Write-Host "--------------------------------------------------"
Write-Host "Upgrading the canister on the local network"
wsl --% . ~/.local/share/dfx/env; dfx deploy --upgrade-unchanged

#######################################################################
Write-Host " "
Write-Host "--------------------------------------------------"
Write-Host "Running the smoketests with pytest for tests marked with 'run_after_upgrade'"
pytest --network=local -m run_after_upgrade
...
if ($pytestExitCode -ne 0) {
    # Print the exit code
    Write-Output "Pytest after canister upgrade had a failure - Exit code: $pytestExitCode"
